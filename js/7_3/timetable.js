...
(function () {
  ...
  const W = 800, H = 480;   // khổ màn 7.3" sáu màu
  ...
  // app này chỉ phục vụ DIY-7_3 (driver 0A) — không có nhánh màn 4 màu
  function is4c() { return false; }
  function devName() {
  ...
  // vẽ bảng vào một context 400x300 (dùng cho cả ô xem trước lẫn thẻ gallery)
  function render(x, now, fourColor) {
    ...
    if (nAM + nPM === 0) {
      vfont(x, true);
      vtext(x, 'Chưa có thời khóa biểu', W / 2, 130, BK);
      vfont(x, false);
      vtext(x, 'Nhập bảng ở mục «Thời khóa biểu»', W / 2, 162, BK);
      vtext(x, 'của webtool rồi bấm «Gửi lên thiết bị»', W / 2, 186, BK);
      return;
    }
    ...
    if (st.header) {
      ...
      vfont(x, true); vleft(x, 'THỜI KHÓA BIỂU', 10, 17, '#fff');
      const ds = DAYS[(now.getDay() + 6) % 7] + ' ' +
        ('0' + now.getDate()).slice(-2) + '/' + ('0' + (now.getMonth() + 1)).slice(-2);
      vfont(x, false); vleft(x, ds, W - 46 - ds.length * ADV, 17, '#fff');
      // giờ căn giữa, viết THẲNG chữ trắng lên dải đỏ: panel Spectra 6 không có
      // partial update nên không cần ô nền trắng như bản 4.2" (máy làm mới toàn
      // màn theo giờ, vì vậy phút ở đây luôn là 00)
      const ts = ('0' + now.getHours()).slice(-2) + ':00';
      vfont(x, true); vleft(x, ts, Math.round((W - ts.length * ADV) / 2), 17, '#fff');
      x.strokeStyle = '#fff'; x.lineWidth = 1.2; x.strokeRect(W - 34, 12, 20, 10);
      x.fillStyle = '#fff'; x.fillRect(W - 36, 16, 2, 2); x.fillRect(W - 32, 14, 16, 6);
    }

    const hdrH = 32, bandH = 22;   // khớp DrawTimetable của firmware 7.3"
    const gTop = top + hdrH;
  ...
  function draw() {
    ...
    // chỉ vẽ lại ĐÚNG thẻ mode 24 trong gallery (gõ một phím mà vẽ lại cả 23
    // thẻ thì máy yếu giật rõ rệt)
    const card = document.querySelector('.mode-card[data-mode="24"] canvas');
    if (card) {
