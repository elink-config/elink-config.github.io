...
/* Máy 2.9" chỉ MỘT tấm (HINK-E029A10-A3, 128×296 bày ngang thành 296×128) —
 * khác máy 2.13" có hai tấm. Vẫn giữ bảng và resIdx để mode_preview.js và bộ
 * dựng «Tự thiết kế» đọc chung một đường, chỉ là bảng có đúng một hàng. */
const RESOLUTIONS = [
  { w: 296, h: 128, label: '296×128', drv: '0f' },
];
let resIdx = 0;
...
/* Đời máy này quảng bá «DIY-2_9N-xxxx» — chữ N tách khỏi ĐỜI CŨ (v1.10,
 * dựng từ HMCLOCK) đang bán dưới tên «DIY-2_9-xxxx». Hai đời nói hai giao
 * thức khác hẳn: đời cũ dùng dịch vụ 16 bit 0xFF00, đời này UUID 128 bit.
 *
 * Tiền tố PHẢI có dấu gạch cuối — để «DIY-2_13N» trơ thì không sao, nhưng để
 * «DIY-2_13» trơ là hộp chọn thiết bị hiện cả máy đời cũ. */
...
function logBleConnectHelp(error) {
  ...
  addLog('1. Đảm bảo thiết bị đã nạp firmware v1.0 trở lên, tên Bluetooth là DIY-2_9N-xxxx');
  addLog('   (máy còn chạy firmware v1.x quảng bá DIY-2_9-xxxx, KHÔNG có chữ N — dùng thẻ «2.9"»)');
...
async function sendAsset() {
  ...
  { const nm = (bleDevice && bleDevice.name) || '';   // chỉ gửi cho đúng dòng máy
    if (nm.indexOf('DIY-2_9N-') !== 0) return; }
  assetBusy = true;
  ...
  try {
    ...
    const r = await fetch('OTA%20firmware/2_9n/asset.bin?v=' + Date.now());
  ...
  if (gattServer != null && gattServer.connected) {
    ...
    try {
      ...
      if (e.name === 'NotFoundError') {
        addLog("Không tìm thấy thiết bị E-Ink 2.9\" đời mới (tên DIY-2_9N-xxxx)");
      } else if (e.message) {
...
const LOCAL_KEY = 'epd_2_9n_fields';
