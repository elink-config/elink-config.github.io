...
(function () {
  ...
  // màn 10.2 CHƯA có firmware cũ ngoài thị trường (v0.1 là bản đầu)
  ...
  // Âm lịch VN (UTC+7) tính thật cho preview card 14 (port từ webtool 4.2")
  ...
  // mode 14 «Lịch dương + âm» có từ fw >= 0.2 — main.js đặt cờ __fwCal
  function fwCal() { return !!window.__fwCal; }

  function m14(x, now) { // card 13: fw >= 0.2 = Lịch dương + âm; fw cũ = Đếm ngược
    if (fwCal()) {
      ...
      // băng đen trên cùng: thứ (chung cho cả hai lịch) + giờ + pin trắng
    ...
    // firmware cũ (v0.1): giữ preview «Đếm ngược sự kiện»
  ...
  window.refreshModeGallery = function () {
    const t = new Date();
    /* Tra theo data-mode chứ KHÔNG theo chỉ số thẻ: thẻ nào ẩn/hiện hay
     * xen thêm cũng không làm lệch phép tra. */
    document.querySelectorAll('.mode-card').forEach(card => {
