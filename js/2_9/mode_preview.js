/*
 * Mode gallery 2.9" BWR (296x128): canvas previews of every display mode with
 * an apply button, supersampled 2x, mirroring the firmware layouts. Red (RD)
 * marks the elements the BWR firmware draws in red.
...
(function () {
  ...
  function battery(x, bx, by, col, label) {
    ...
    const p = Math.max(0, Math.min(10, Math.round((voltValue() - 2.4) / 0.07)));
  ...
  function m3(x, now, W, H) {
    font(x, 11, 1); x.fillStyle = RD;
    x.fillText('Tháng ' + (now.getMonth() + 1), 6, 14);
    serif(x, 44, 1);
    center(x, now.getDate(), 39, H / 2 + 16, RD);
    font(x, 11, 1);
    ...
    for (let d = 1; d <= maxD; d++) {
      ...
      if (d === now.getDate()) {
        x.fillStyle = RD; x.fillRect(cx - cw / 2 + 1, cy - 1, cw - 2, rh - 1);
